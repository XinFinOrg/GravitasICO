...
class AdminwalletController extends Controller {
	...
	function adminauth($email, $password) {
		$arr = array('XDC_username' => $email, 'status' => 'active');
		$check = Adminwallet::first();
		$check = Adminwallet::where('status','active')->first();
		if ($check) {
	...
	function home() {
		if (Session::get('wallet_adid') == "") {
			...
			$usdt_bal = get_usdt_wallet_info($adminusdtaddr);
...
//            $btc_bal1 = "qqqqq";
			...
			return view('wallet.home', ['xdc_bal' => $xdc_bal,'xdce_bal'=>$xdce_bal, 'eth_bal' => $eth_bal, 'btc_bal' => $btc_bal1,'usdt_bal' => $usdt_bal1,'bch_bal' => $bch_bal1, 'xrp_bal' => $xrp_bal1]);
		}
	}
...
//                }
		...
		if (Session::get('wallet_adid') == "") {
			...
			if ($request->isMethod('post')) {
				...
				if ($request['curr_pass'] != "") {
					...
					if (Hash::check($request['curr_pass'], $upt->XDC_password)) {
	...
	function profit(Request $request) {
		if (Session::get('wallet_adid') == "") {
            ...
            $profit_bch = Profit::where('theftCurrency', 'BCH')->sum('theftAmount');
			$profit_xrp = Profit::where('theftCurrency', 'XRP')->sum('theftAmount');

			return view('wallet.profit', ['result' => $result, 'profit_eth' => $profit_eth, 'profit_btc' => $profit_btc,'profit_bch' => $profit_bch, 'profit_xrp' => $profit_xrp]);
		}
	}
